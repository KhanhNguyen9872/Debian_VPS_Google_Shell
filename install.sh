...
sudo apt install libc-bin base-files sysvinit-utils tzdata bazel apt-transport-https xfce4 xarchiver firefox-esr mesa-utils git xfce4-goodies pv nmap nano apt-utils dialog terminator autocutsel dbus-x11 dbus neofetch perl p7zip unzip zip curl tar git python3 python3-pip net-tools openjdk-17-jdk openssl libreoffice hardinfo docker-compose tigervnc-standalone-server tigervnc-xorg-extension docker -y
export HOME="$(pwd)"
export DISPLAY=":0"
cd $HOME 2> /dev/null
